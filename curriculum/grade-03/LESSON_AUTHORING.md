...
- id: vocabulary
  ...
  - order: 2
    target: how
    say: |-
      HOW dùng để hỏi như thế nào. [long pause]
      Listen first! HOW. [long pause]
      Your turn now! Can you say: How?
    accept: Chấp nhận con nói “how”; chưa cần dùng trong câu.
- id: patterns
  steps:
  - order: 3
    target: ask
    say: "How are you? [long pause] Now you ask me!"
...
Lời cô sau lượt học sinh cũng đặt trong `more` với `say` và không có `accept`. Hệ thống giữ phản hồi tự nhiên cho câu trả lời và chuyển sang câu cố định tiếp theo. Mặc định một mục cho con tối đa 2 lần trả lời; nếu cần khác, thêm `attempts: 1` (từ 1 đến 5) ngay dưới `order`. Các trường nội bộ như objective ID, stage ID và luật chuyển trạm được quản lý bởi code.

Lời khen phụ thuộc vào bằng chứng, nhất là câu khẳng định đã nghe rõ một âm, không đặt thành câu cố định nếu hệ thống chỉ có bản chép lời. Teacher hiện tại tạo lời nhận xét sau khi LLM đánh giá câu con nói.
