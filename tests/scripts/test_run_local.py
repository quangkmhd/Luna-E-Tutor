...
def test_local_runner_starts_and_cleans_up_all_four_processes():
    ...
    assert 'kill "${backend_pid:-}" "${voice_pid:-}" "${talk_pid:-}" "${web_pid:-}"' in script
...
def test_local_runner_keeps_separate_logs_for_all_four_services():
    script = (ROOT / "scripts/run-local.sh").read_text()
