...
class GoogleCaptionSource:
    """The authored line and language span currently entering Google TTS."""
        ...
        if isinstance(frame, (InterruptionFrame, ErrorFrame, CancelFrame)):
            ...
            if frame.context_id == self._context_id:
                if self._audio_delivered:
                    ...
                    await self.push_frame(ErrorFrame('TTS ended without delivered audio'), FrameDirection.UPSTREAM)
                self._context_id = None
                self._audio_delivered = False
...
class LanguageTaggedTTSProcessor(FrameProcessor):
    """Switch language only after output confirms the current segment was played."""

    def __init__(self, *, provider: str = 'soniox', google_vi_voice: str = '',
                 google_en_voice: str = '', caption_source: GoogleCaptionSource | None = None):
        super().__init__()
        self._provider = provider
        self._google_voices = {'vi': google_vi_voice, 'en': google_en_voice}
    ...
    async def _next_segment(self) -> None:
        ...
        if self._caption_source is not None:
            self._caption_source.set_segment()
        self._active_context_id = None
        language = Language.VI if span.language == 'vi' else Language.EN
        if self._provider == 'google':
            from pipecat.services.google.tts import GoogleTTSService

            delta = GoogleTTSService.Settings(
                language=language, voice=self._google_voices[span.language],
                speaking_rate=0.9 if span.language == 'en' else 1.0,
            )
        else:
            delta = SonioxTTSSettings(language=language)
        await self.push_frame(TTSUpdateSettingsFrame(delta=delta))
    ...
    async def _start_next(self) -> None:
        ...
        if self._caption_source is not None:
            self._caption_source.set_source(self._active.text)
        self._remaining = deque(
            SpeechSegment(span.language, cleaned)
            for span in parse_speech_segments(self._active.text, strict=False)
            if (cleaned := tts_speech_text(span.text))
        )
        if self._remaining:
            await self._next_segment()
        else:
            # No audio can be delivered for a cue-only generated response.
            self._active = None
            await self.push_frame(ErrorFrame('TTS text has no speakable content'))
