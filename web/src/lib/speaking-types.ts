...
export type SpeakingState = {session_id: string; config: Config; version: number; status: 'active'|'completed'; level: number; independent_streak: number; difficulty_streak: number; word_evidence: WordUse[]; last_delivered_text: string; opening_message: string};
