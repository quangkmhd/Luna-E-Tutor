import sys,asyncio,json,tempfile,argparse
...
ANSWERS={
'warm-up.hello':'Hello Luna.', 'warm-up.feelings':"I'm fine, thank you.", 'warm-up.start':"Let's start.",
'lesson-01.place-meaning':'A city has tall buildings. The countryside has many trees.',
'lesson-01.class':"I'm in Class 5B.",'lesson-01.home':'I live countryside.',
'lesson-01.ask-luna':'Where do you live?',
'lesson-02.favourite-animal':"It's a dolphin.",'lesson-02.favourite-colour':"It's pink.",
'lesson-02.favourite-food':"It's a sandwich.",'lesson-02.favourite-sport':"It's table tennis.",
...
'level-02.birthday':"It's in May.",'level-02.hobby':"My hobby is playing table tennis.",
'level-02.subject':"My favourite subject is English.",'level-02.ask-luna':"What's your hobby?",
...
async def main(args):
 out=args.output
 if out.exists(): raise RuntimeError('Will not overwrite')
 payload={'scope':'scripted student full Unit1 API Pipecat journey; semantic review pending','complete':False,'records':[]}
 values=dotenv_values(args.env_file)
 with tempfile.TemporaryDirectory() as folder:
  app=build_runtime_app({'OPENROUTER_API_KEY':values['OPENROUTER_API_KEY'],'TUTOR_DATABASE_PATH':str(Path(folder)/'session.sqlite3')},turn_service_adapter=PipecatTurnService)
  async with app.router.lifespan_context(app):
   async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app),base_url='http://test') as api:
    current=(await api.post('/api/sessions')).json();visits={}
    for i in range(100):
     aid=current['activity_id'];visits[aid]=visits.get(aid,0)+1
     if visits[aid]>3:
     ...
     current=response.json()['session'];record['result']={k:v for k,v in current.items() if k!='messages'};record['teacher']=current['messages'][-1];payload['records'].append(record)
     out.write_text(json.dumps(payload,indent=2,ensure_ascii=False))
     print(i,aid,'->',current['activity_id'],flush=True)
     if aid=='free-talk.conversation':
      payload['reached_free_talk']=True;break
    payload['complete']=True;out.write_text(json.dumps(payload,indent=2,ensure_ascii=False))
if __name__ == '__main__':
 parser=argparse.ArgumentParser(description='Diagnostic full Unit 1 scripted-student text journey. No automatic semantic acceptance.')
