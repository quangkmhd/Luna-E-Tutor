...
- **Quy tắc hiện tại:** Trong một lượt nói bình thường, chỉ hành động **Gửi** của học sinh mới nộp câu trả lời. Trẻ ngừng giữa câu không làm hệ thống kết thúc lượt, gọi Jev hoặc phát lời Luna.
- Dùng chế độ Pipecat của `SonioxSTTService` (`vad_force_turn_endpoint=True`) để tắt Soniox tự phát hiện điểm kết thúc. Pipecat gửi lệnh `finalize` cho Soniox ngay khi dịch vụ nhận `VADUserStoppedSpeakingFrame`; Soniox tạo transcript `final` và Pipecat chuyển thành `TranscriptionFrame(finalized=True)`.
...
- **Nộp lượt:** học sinh ấn Gửi một lần; trình duyệt tắt mic ngay, khóa Gửi/mic và gửi lệnh nộp kèm ID lượt qua Pipecat/RTVI, không chèn thời gian chờ cố định. Server kiểm tra ID và trạng thái rồi đẩy `VADUserStoppedSpeakingFrame` ngược tới Soniox STT. Pipecat gửi `finalize` ngay trên kết nối Soniox đang mở.
- **Dùng transcript `final`:** Soniox xử lý lệnh chốt rồi trả transcript `final`; chỉ `TranscriptionFrame(finalized=True)` mới được đưa vào Jev đúng một lần. Không lấy chữ `interim` trên web để đánh giá. `final` đến sau khi lượt đã nộp hoặc lệnh Gửi trùng không tạo lượt Jev mới. Thời gian từ lúc bấm Gửi đến lúc Jev chạy gồm độ trễ Soniox trả `final` và mạng; không thể đồng thời đòi bản `final` và coi nó đã có sẵn ngay tại thời điểm bấm.
...
- Voice dùng Soniox STT ở chế độ Pipecat endpoint; nút Gửi chuyển ngay `VADUserStoppedSpeakingFrame` tới Soniox để yêu cầu `finalize`. Voice chờ Soniox trả `TranscriptionFrame(finalized=True)` rồi nộp cho Backend đúng một lần, không có cổng chờ khung audio hoặc khoảng chờ cố định ở web. Voice vẫn xác nhận toàn bộ TTS trước khi bật quyền mở mic lại. Text gửi API trực tiếp, không phát TTS. Web tắt mic khi Gửi và giữ tắt sau khi Luna phát xong.
...
| 2026-09-24 | Người dùng yêu cầu bấm Gửi là đưa text đang có cho LLM ngay, không chờ âm thanh ngừng hoặc Soniox `final`. | Web gửi transcript mới nhất trong `luna.submit-turn`; Voice gọi Backend ngay nếu transcript có chữ, đồng thời yêu cầu Soniox chốt lượt STT. Bỏ chờ 250 ms trên web và cổng chờ âm thanh trên server. `final` đến muộn bị bỏ qua để Jev chỉ chạy một lần. Nếu chưa có chữ, vẫn chờ Soniox `final` có giới hạn. Quyết định này thay quy tắc chờ `final` trong dòng chốt phương án 3 ở trên. |
| 2026-09-24 | Người dùng làm rõ rằng Gửi phải dùng cơ chế Pipecat lấy transcript `final`, không nộp `interim` đang hiển thị. | Bỏ nhánh gửi text từ trình duyệt. Nút Gửi gửi lệnh RTVI ngay; Voice đẩy `VADUserStoppedSpeakingFrame` để Pipecat gửi `finalize` tới Soniox ngay; đợi `TranscriptionFrame(finalized=True)` rồi mới gọi Jev. Không tái tạo cổng chờ khung audio gây treo. Quyết định này thay dòng ngay phía trên. |
