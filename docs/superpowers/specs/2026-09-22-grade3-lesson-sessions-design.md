...
Other examples of `accept` criteria are `Đếm tiếp đến ten; sai 1–2 số vẫn đạt.` and `Hi/Hello + I'm + tên; thiếu Hi vẫn đạt.` These define successful answers, not hints to give the learner.
...
Enter the three stations in order. Deliver a teacher script turn verbatim when its turn becomes active. Stop at each learner opportunity, send the utterance and its `accept` success criterion to the current evaluator, and use existing feedback and retry decisions. The evaluator must mark success when the learner's actual response satisfies that criterion, even if it omits an explicitly optional word or has an explicitly tolerated small error; examples are not exact-match requirements. Advance only after the required opportunity is handled according to the existing completion rule. Teacher feedback after a learner response may be naturally phrased; it must not skip, rewrite, or duplicate the next fixed script turn. On a support limit, advance with an honest transition rather than claiming success.
