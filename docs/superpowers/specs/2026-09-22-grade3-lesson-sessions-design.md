...
Lesson YAML has `lesson`, `title`, a required `greeting`, short `words` and `patterns` definitions, and exactly three ordered `stations`: `vocabulary`, `patterns`, `conversation`. The greeting has `order: 1`; each teaching step has the next lesson-wide order number, so `hello` is 2, `hi` is 3, and so on across all stations. A step groups the teacher lines for one teaching item in `say` and the learner opportunity that follows in `accept`; `target` refers to a word or pattern key. `accept` is a natural-language description for the evaluator LLM: it teaches the LLM what learner responses to regard as acceptable in this context, including allowed omissions or small errors. It is based on the workbook's `Đáp án chấp nhận` column, not its silence or correction columns, and is never spoken by the teacher. Teacher-only steps omit `accept`. The number identifies a teaching item, not every sentence or learner reply. Workbook row numbers are not stored in the lesson YAML. Teacher text is the sheet's wording, with `[long pause]` inserted only at suitable spoken boundaries; preserve the wording, case, and punctuation.
  ...
  - id: vocabulary
    steps:
      - order: 2
        ...
        target: hello
        accept: Chấp nhận “hello” dùng như lời chào; có âm /h/ đầu từ.
      - order: 3
        say: |-
          ...
          Your turn now! Hi!
        target: hi
        accept: Chấp nhận “hi” dùng như lời chào.
  - id: patterns
    steps: []
  - id: conversation
    steps: []
...
Other examples of `accept` descriptions are `Đếm tiếp đến ten; sai 1–2 số vẫn đạt.` and `Hi/Hello + I'm + tên; thiếu Hi vẫn đạt.` They guide the evaluator LLM's judgment; they are not hints to give the learner or literal strings to match.
...
Enter the three stations in order. Deliver a teacher script turn verbatim when its turn becomes active. Stop at each learner opportunity and include its `accept` description in the evaluator LLM request alongside the learner's actual response and objective context. The LLM interprets that description to judge contextual acceptability, including stated tolerances; application code does not parse `accept` into hard-coded matching rules. Use existing feedback and retry decisions. Advance only after the required opportunity is handled according to the existing completion rule. Teacher feedback after a learner response may be naturally phrased; it must not skip, rewrite, or duplicate the next fixed script turn. On a support limit, advance with an honest transition rather than claiming success.
